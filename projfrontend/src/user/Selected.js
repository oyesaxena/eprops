...
class Selected extends Component {
  ...
  postUserImages=()=>{
    ...
    axios.post("http://3.7.254.186:8000/selectedImages/"+ userData.user._id,{userId:userData.user._id,offset:this.state.offset})
    .then((response)=>{
      const data = response.data
      this.setState({ images: data, loading: false });
      this.setState({
        pageCount: Math.ceil(this.state.images.selectedImagesCount / this.state.postsPerPage),
  ...
  getUserImages = () => {
    const userData = JSON.parse(localStorage.jwt);
    axios
      .get("http://3.7.254.186:8000/selectedImages/" + userData.user._id)
      .then((response) => {
  ...
  displayImages = (imageData) => {
    ...
    const currentPosts = selectedImages.slice(this.state.offset,
    ...
    return selectedImages.map((image, index) => (
      <div key={index} className=".col-sm-4">
        <img
          ...
          src={image.location}
          style={{ height: "300px", width: "300px" , margin:"10px 10px", borderColor:"#ffff00",borderStyle:"solid", }}
        ...
        <Modal isOpen={selectedPost === image} className="mt-2" style={{maxWidth:"600px"}}>
          <div className="modal-header bg-warning">
            <h3 className="modal-title" style={{textAlign:"center",width:"550px",wordWrap:"break-word" }} id="exampleModalLabel">
